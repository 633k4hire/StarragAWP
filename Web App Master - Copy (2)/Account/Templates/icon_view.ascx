<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="icon_view.ascx.cs" Inherits="Web_App_Master.Account.Templates.icon_view" %>
<!--Icon Template-->
...
<span id="InOutIndicator"  runat="server" class="edge-badge fg-white shadow-metro-black icon-badge " style="z-index:3 !important;"><strong><%# Eval("AssetNumber")%></strong></span></div>
    <div runat="server" id="DownloadLink" class="i-download" style="margin-top:-4px;padding-left:2px">         <i title="Add To Checkout" id="dl_link"  style="font-size:1.5em;" class="glyphicon glyphicon-plus c-lime av-add-cursor" onclick="<%# prepareAjax(Eval("AssetNumber").ToString(),Eval("IsOut").ToString())%>"></i>
</div>
