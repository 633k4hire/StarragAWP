...
<div class="list shadow" style="overflow:hidden; text-align:left" onclick="BarcodeScanned('<%# Eval("AssetNumber")%>', '<%# Eval("IsHistoryItem")%>','<%# Eval("DateShippedTicks")%>');">
    <span id="InOutIndicator" runat="server" class="list-title fg-white shadow-metro-black" style="margin-left:5px;padding-left:5px;"><strong><%# Eval("AssetNumber") %> - <%# Eval("AssetName") %> - <%# Eval("DateShippedString") %> - <%# Eval("DateRecievedString") %></strong></span>
</div>
